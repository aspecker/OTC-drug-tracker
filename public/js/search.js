	...
	$.ajax({
		...
		Object.keys(obj).forEach(function(key) {
			...
			$("#searchResults").append(
				`<tr><td>${brandName}</td><td>${genericName}</td><td>${indications}</td><td>${activeIngredient}</td><td><a class="loginAnchor"><button class="add-btn btn btn-danger btn-block" id="${key}" value='${pullData}'>Click</button></a></td></tr><`
			);
		});
		check();
	})
